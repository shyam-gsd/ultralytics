    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                if isinstance(m,(QuantConv, QDWConv)):
                    m.conv = fuse_conv_and_bn(m.conv, m.bn,True)  # update conv
                    delattr(m, "bn")  # remove batchnorm
                    m.forward = m.forward_fuse  # update forward
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    if isinstance(model, OrderedDict):
        nmodel = DetectionModel()
        nmodel.load_state_dict(state_dict=model, strict=False)  # FP32 model
        nmodel.to(device).float()  # load to device
        model = nmodel
    else:
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect,QuantDetect)):
                return "detect"
